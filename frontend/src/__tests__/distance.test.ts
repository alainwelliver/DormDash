...
describe("distance utils", () => {
  ...
  test("formatDistanceMiles formats short and long values", () => {
    ...
    expect(formatDistanceMiles(undefined)).toBe("N/A");
    expect(formatDistanceMiles(Number.NaN)).toBe("N/A");
  });
  ...
  test("estimateEtaMinutes derives rounded-up minutes", () => {
    ...
    expect(estimateEtaMinutes(0, 12)).toBe(0);
    expect(estimateEtaMinutes(-1, 12)).toBe(0);
  });
